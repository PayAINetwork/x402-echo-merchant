...
const MAINNET_ENDPOINTS: EndpointInfo[] = [
  {
        ...
        <Card className="w-full bg-card border border-border shadow-none mb-4 mt-4">
          <CardContent className="py-4 px-4 sm:py-6 sm:px-6 flex flex-col gap-4">
            ...
            {MAINNET_ENDPOINTS.map(ep => (
              <EndpointCard key={ep.label} ep={ep} />
            ))}
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-1 flex-shrink-0">
            <a href={ep.url} target="_blank" rel="noopener noreferrer">
              <ExternalLink className="w-4 h-4 text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300" />
            </a>
            ...
            {ep.supportsPermit2 && (
              <Button
                size="icon"
                variant="ghost"
                className="opacity-70 hover:opacity-100 transition-all h-8 w-8"
                onClick={() => setExpanded(prev => !prev)}
                aria-label="Toggle transfer methods"
              >
                <ChevronDown
            ...
            <div className="flex items-center justify-between gap-2">
              <div className="flex items-center gap-2 min-w-0 flex-1">
                ...
                <a
                  ...
                  className="text-amber-600 dark:text-amber-400 underline hover:text-amber-800 dark:hover:text-amber-300 text-xs font-medium truncate min-w-0 font-mono"
                >
                  {ep.url.replace(API_URL || '', '')}?assetTransferMethod=permit2
                </a>
              </div>
              <div className="flex items-center gap-1 flex-shrink-0">
                <a href={permit2Url} target="_blank" rel="noopener noreferrer">
                  <ExternalLink className="w-3.5 h-3.5 text-amber-600 dark:text-amber-400 hover:text-amber-800 dark:hover:text-amber-300" />
                </a>
