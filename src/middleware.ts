...
function buildCorsHeaders(request: NextRequest): Headers {
  ...
  // Expose PAYMENT-RESPONSE header to the browser
...
export async function middleware(request: NextRequest) {
  ...
  if (pathname.startsWith('/api/xlayer/')) {
    const requestedAmount = await getRequestedAmount(request, xlayerConfig.price);
  ...
  if (pathname.startsWith('/api/xlayer-testnet/')) {
    const requestedAmount = await getRequestedAmount(request, xlayerTestnetConfig.price);
 ...
 * Uses PAYMENT-SIGNATURE/PAYMENT-RESPONSE headers and CAIP-2 network format
 *
...
export function paymentMiddleware(
  ...
  return async function middleware(request: NextRequest) {
    ...
    const { amount, asset } = atomicAmountForAsset;
    ...
    // svm networks
      ...
      paymentRequirements.push({
        ...
        extra: {
          feePayer,
          // Include compatibility fields for paywall display
          description: description ?? '',
          mimeType: mimeType ?? '',
          resource: resourceUrl,
          outputSchema: {
