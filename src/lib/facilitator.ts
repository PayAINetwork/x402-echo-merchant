...
function extractErrorMessage(
	data: FacilitatorErrorResponse | null,
	fallback: string,
): string {
	...
	if (data.errorReason) {
		return data.errorMessage
			? `${data.errorReason}: ${data.errorMessage}`
			: data.errorReason;
	}
...
export function useFacilitator(config?: FacilitatorConfig): FacilitatorHooks {
	...
	const getHeaders = async (
		...
		if (config?.createAuthHeaders) {
			...
			const typeHeaders =
				type === "verify" ? authHeaders.verify : authHeaders.settle;
	...
	return {
		verify: async (paymentPayload, paymentRequirements) => {
			...
			const data = await safeJson<VerifyResponse | FacilitatorErrorResponse>(
				res,
			);
		...
		settle: async (paymentPayload, paymentRequirements) => {
			...
			const data = await safeJson<SettleResponse | FacilitatorErrorResponse>(
				res,
			);
		...
		supported: async () => {
			...
			const data = await safeJson<SupportedResponse | FacilitatorErrorResponse>(
				res,
			);
